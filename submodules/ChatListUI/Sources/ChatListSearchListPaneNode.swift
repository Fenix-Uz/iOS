...
public enum ChatListSearchEntryStableId: Hashable {
    ...
    case novagramAdPeerId(EnginePeer.Id)
...
public enum ChatListSearchEntry: Comparable, Identifiable {
    ...
    case novagramAdPeer(EnginePeer, String, Int, PresentationTheme, PresentationStrings, String?)
    ...
    public var stableId: ChatListSearchEntryStableId {
        ...
        case let .novagramAdPeer(peer, _, _, _, _, _):
    ...
    public static func == (lhs: ChatListSearchEntry, rhs: ChatListSearchEntry) -> Bool {
        ...
        case let .novagramAdPeer(lhsPeer, lhsOrderId, lhsIndex, lhsTheme, lhsStrings, lhsQuery):
            if case let .novagramAdPeer(rhsPeer, rhsOrderId, rhsIndex, rhsTheme, rhsStrings, rhsQuery) = rhs, lhsPeer == rhsPeer && lhsOrderId == rhsOrderId && lhsIndex == rhsIndex && lhsTheme === rhsTheme && lhsStrings === rhsStrings && lhsQuery == rhsQuery {
                return true
            } else {
                return false
            }
    ...
    public static func < (lhs: ChatListSearchEntry, rhs: ChatListSearchEntry) -> Bool {
        switch lhs {
        case let .novagramAdPeer(_, _, lhsIndex, _, _, _):
            if case let .novagramAdPeer(_, _, rhsIndex, _, _, _) = rhs {
                return lhsIndex <= rhsIndex
            } else {
                return true
            }
        case let .topic(_, _, lhsIndex, _, _, _):
            if case let .topic(_, _, rhsIndex, _, _, _) = rhs {
                return lhsIndex <= rhsIndex
            } else if case .novagramAdPeer = rhs {
                return false
            } else {
                return true
            }
        ...
        case let .localPeer(_, _, _, lhsIndex, _, _, _, _, _, _, _, _):
            switch rhs {
            case .topic, .recentlySearchedPeer, .novagramAdPeer:
                return false
        ...
        case let .adPeer(_, lhsIndex, _, _, _, _, _, _):
            switch rhs {
            case .topic, .recentlySearchedPeer, .localPeer, .novagramAdPeer:
                return false
        ...
        case let .globalPeer(_, _, lhsIndex, _, _, _, _, _, _, _, _):
            switch rhs {
            case .topic, .recentlySearchedPeer, .localPeer, .adPeer, .novagramAdPeer:
                return false
...
final class ChatListSearchListPaneNode: ASDisplayNode, ChatListSearchPaneNode {
    ...
    init(context: AccountContext, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, interaction: ChatListSearchInteraction, key: ChatListSearchPaneKey, peersFilter: ChatListNodePeersFilter, requestPeerType: [ReplyMarkupButtonRequestPeerType]?, location: ChatListControllerLocation, searchQuery: Signal<String?, NoError>, searchOptions: Signal<ChatListSearchOptions?, NoError>, navigationController: NavigationController?, parentController: ViewController?, globalPeerSearchContext: GlobalPeerSearchContext?) {
        ...
        if enableRecentlySearched {
            ...
            |> map { peers -> [RecentlySearchedPeer] in
                ...
                _ = previousRecentlySearchedPeerOrder.modify { current in
        ...
        |> mapToSignal { [weak self] query, approvedGlobalPostQueryState, options, searchScope, downloadItems, _, _ -> Signal<([ChatListSearchEntry], Bool, String?)?, NoError> in
            ...
            if case .savedMessagesChats = location {
                ...
                _ = previousRecentlySearchedPeersState.swap(nil)
            }

            let foundRemotePeers: Signal<([FoundPeer], [FoundPeer], [AdPeer], Bool, FenixNovagramPromotedChannel?), NoError>
            let currentRemotePeersValue: ([FoundPeer], [FoundPeer], [AdPeer]) = currentRemotePeers.with { $0 } ?? ([], [], [])
            if case .savedMessagesChats = location {
                foundRemotePeers = .single(([], [], [], false, nil))
            } else if let query = query, case .chats = key {
                if query.hasPrefix("#") || options?.folder != nil {
                    foundRemotePeers = .single(([], [], [], false, nil))
                } else {
                    foundRemotePeers = (
                        .single((currentRemotePeersValue.0, currentRemotePeersValue.1, currentRemotePeersValue.2, true, nil))
                        |> then(
                            globalPeerSearchContext.searchRemotePeers(engine: context.engine, query: query)
                            |> mapToSignal { result in
                                return combineLatest(
                                    context.engine.peers.searchAdPeers(query: query),
                                    FenixNovagramSearchAds.promotedChannel(context: context, query: query)
                                )
                                |> map { adPeers, novagramPromoted in
                                    return (result.0, result.1, adPeers, false, novagramPromoted)
                                }
                            }
                        )
                    )
            ...
            |> map { accountPeer, foundLocalPeers, foundRemotePeers, foundRemoteMessages, foundPublicMessages, presentationData, searchState, selectionState, resolvedMessage, recentPeers, allAndFoundThreads, adsHidden -> ([ChatListSearchEntry], Bool, String?)? in
                ...
                _ = currentRemotePeers.swap((foundRemotePeers.0, foundRemotePeers.1, foundRemotePeers.2))
                ...
                // Fenixuz: Novagram promoted channel — always the very top row of global
                // search, above Telegram's own sponsored row. `foundRemotePeers.4` is nil
                // (no row) when our backend returns no ad (HTTP 404) for this query/viewer.
                if !adsHidden, let novagramPromoted = foundRemotePeers.4, !existingPeerIds.contains(novagramPromoted.peer.id) {
                    existingPeerIds.insert(novagramPromoted.peer.id)
                    entries.insert(.novagramAdPeer(novagramPromoted.peer, novagramPromoted.orderId, 0, presentationData.theme, presentationData.strings, finalQuery), at: 0)
                }
                ...
                if !foundRemotePeers.3 {
                    ...
                    if foundRemoteMessages.1 && (searchScope != .everywhere || hadAnySearchMessagesBefore) {
                        ...
                        if hasAnyMessages {
                            _ = hadAnySearchMessages.swap(true)
                        } else {
                            switch searchScope {
                            case .everywhere:
                                break
        ...
        openMediaMessageImpl = { message, mode in
            _ = context.sharedContext.openChatMessage(OpenChatMessageParams(context: context, chatLocation: nil, chatFilterTag: nil, chatLocationContextHolder: nil, message: message._asMessage(), standalone: false, reverseMessageGalleryOrder: true, mode: mode, navigationController: navigationController, dismissInput: {
        ...
        let chatListInteraction = ChatListNodeInteraction(context: context, animationCache: self.animationCache, animationRenderer: self.animationRenderer, activateSearch: {
            ...
            switch location {
            case .chatList, .forum:
                _ = context.engine.peers.addRecentlySearchedPeer(peerId: peer.id).startStandalone()
            ...
            case let .peer(peerData):
                if let peer = peerData.peer.peer, let message = peerData.messages.first {
                    _ = context.engine.peers.ensurePeerIsLocallyAvailable(peer: peer).startStandalone()
        ...
        }, openUrl: { url, _, _, _ in
            interaction.openUrl(url)
        }, openInstantPage: { [weak self] message, _ in
            ...
            if let strongSelf = self {
                ...
                searchOptions: strongSelf.searchOptionsValue, messageContextAction: { message, node, rect, gesture, paneKey, downloadResource in
                    ...
                    items.append(ActionSheetButtonItem(title: strongSelf.presentationData.strings.DownloadList_ClearDownloadList, color: .destructive, action: { [weak actionSheet] in
                        ...
                        _ = clearRecentDownloadList(postbox: strongSelf.context.account.postbox).startStandalone()
                    }))
                    ...
                    _ = ((strongSelf.context.fetchManager as! FetchManagerImpl).entriesSummary
                    |> take(1)
        ...
        let toggleChannelsTabExpanded: () -> Void = {
            _ = (isChannelsTabExpandedValue.get() |> take(1)).startStandalone(next: { value in
        ...
        if case .channels = key {
            ...
            _ = self.context.engine.peers.requestGlobalRecommendedChannelsIfNeeded().startStandalone()
            ...
            |> mapToSignal { localChannels, remoteChannels -> Signal<RecentItems, NoError> in
                ...
                |> map { peers, notificationSettings, _, storyStats, readCounters, participantCounts, globalNotificationSettings -> RecentItems in
                    /*#if DEBUG
            ...
            _ = self.context.engine.peers.requestRecommendedAppsIfNeeded().startStandalone()
            ...
            |> mapToSignal { localApps, remoteApps -> Signal<RecentItems, NoError> in
                ...
                |> map { peers, notificationSettings, _, storyStats, readCounters, globalNotificationSettings -> RecentItems in
                    var result: [ChatListRecentEntry] = []
                    var existingIds = Set<PeerId>()
        ...
        |> deliverOnMainQueue).startStrict(next: { [weak self] presentationData, recentItems in
            if let strongSelf = self {
                ...
                let transition = chatListSearchContainerPreparedRecentTransition(from: previousRecentItems?.entries ?? [], to: recentItems.entries, forceUpdateAll: forceUpdateAll, context: context, presentationData: presentationData, filter: peersFilter, key: key, peerSelected: { peer, threadId, isRecommended, action in
                    ...
                    _ = context.engine.peers.removeRecentlySearchedPeer(peerId: peerId).startStandalone()
                        ...
                        linkAction: { attributes, _ in
                            ...
                            if let value = attributes[NSAttributedString.Key(rawValue: TelegramTextAttributes.URL)] as? String {
                                if !value.isEmpty {
                                    ...
                                    _ = (context.engine.peers.resolvePeerByName(name: "botfather", referrer: nil)
    ...
    @objc private func animationTapGesture(_ recognizer: UITapGestureRecognizer) {
        if case .ended = recognizer.state, !self.emptyResultsAnimationNode.isPlaying {
            _ = self.emptyResultsAnimationNode.playIfNeeded()
        }
    }
        ...
        if let (item, previousItem, nextItem, order, type, _) = self.playlistStateAndType {
            ...
            if let (mediaAccessoryPanel, mediaType) = self.mediaAccessoryPanel, mediaType == type {
                ...
                mediaAccessoryPanel.setRate = { [weak self] rate, changeType in
                    ...
                    _ = (strongSelf.context.sharedContext.accountManager.transaction { transaction -> AudioPlaybackRate in
